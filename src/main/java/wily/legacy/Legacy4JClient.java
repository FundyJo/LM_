...
public class Legacy4JClient {
    ...
    public static void init() {
        ...
        // MinigamesController.init() is already called from Legacy4J.init()
        // which runs on both client and server sides via the main entrypoint
    ...
    public static MinigamesController getController() {
        ...
        if (level == null) return new MinigamesController();
        return MinigamesController.getMinigameController(level);
    }

    public static Minigame<?> getMinigame() {
        ClientLevel level = Minecraft.getInstance().level;
        if (level == null) return Minigame.NONE;

        MinigamesController controller = MinigamesController.getMinigameController(level);
        Minigame<?> minigame = controller.getActiveMinigame();
        return minigame;
    }
